...
**Architecture:** Bugs are stored in `.claude-context/buglog.json` (versioned JSON envelope). A token-overlap matcher (`pkg/match`) compares new code against stored bad-code snippets — overlap ≥ 3 non-stop tokens triggers a warning. The prewrite hook runs cerebrum and buglog passes in sequence and emits a combined warning block. The postwrite hook auto-appends bugfix-classified edits with old_string ≥ 5 tokens. The `buglog` CLI provides add/list/clear commands.
...
Expected: FAIL with `cannot find package "github.com/ranwei/claude-context/pkg/match"`.
...
Usage:
  ...
  claude-context cerebrum <cmd>    manage per-project cerebrum rules
