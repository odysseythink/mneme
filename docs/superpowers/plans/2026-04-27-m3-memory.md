...
**Goal:** Implement session memory — classifying edits per turn, aggregating session summaries on SessionStart, and persisting them to `~/.claude/claude-context-memory.md`.
...
```bash
go build -o ./bin/claude-context ./cmd && ./bin/claude-context --version
```

Expected: `claude-context 0.1.0-m3`
