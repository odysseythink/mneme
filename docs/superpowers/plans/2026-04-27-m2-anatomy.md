...
**Goal:** Build per-file anatomy scanning, write it to `.claude-context/anatomy.md`, and wire the pre-read hook to emit file descriptions and repeat-read warnings to Claude.

**Architecture:** A new `pkg/scanner/` package (walker + per-language extractors) feeds `state.WriteAnatomy` which writes an indexed markdown file. The `runPreRead` hook replacement reads anatomy on every pre-read event and emits info via exit 1. `claude-context init --yes` auto-runs scan after scaffolding.
...
// <projectRoot>/.claude-context/anatomy.md. Also increments scan_count.
...
./bin/claude-context scan --help 2>&1 | head -5
```
...
- [ ] Manual smoke: `CLAUDE_CONTEXT_DEBUG=1 claude` → read a file → see `⚡ claude-context:` in transcript
