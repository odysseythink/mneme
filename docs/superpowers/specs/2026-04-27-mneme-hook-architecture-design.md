# Mneme Hook Architecture Design

**Date:** 2026-04-27
**Status:** Draft
**Scope:** Embed openwolf-style token-saving mechanisms (Claude Code hooks + project memory) into the existing `mneme` Go binary, alongside the unchanged MCP server. One binary, multiple subcommands. Six-milestone delivery.

---

## Problem

The current `mneme` is a stateless MCP server: it answers `index_codebase` / `search_codebase` calls when Claude chooses to invoke them. It has no visibility into Claude's other actions (`Read`, `Edit`, `Write`) and no way to prevent Claude from re-reading the same file ten times in a session, ignoring high-leverage descriptions, or repeating known-bad patterns.
...
We want those mechanisms inside `mneme` — but as part of the existing single-binary deployment, not as a second tool the user has to install.

---

## Goals

- Port all 13 openwolf token-saving mechanisms into `mneme`, organized into 6 deliverable milestones.
...
- One install command (`mneme init`) registers Claude Code hooks, writes the rules file, and creates the per-project state directory.
...
|---|---|---|
| `anatomy.md` | `<project>/.mneme/` | yes (team baseline) |
| `cerebrum.md` | `<project>/.mneme/` | yes (curated rules) |
| `buglog.json` | `<project>/.mneme/` | optional (auto-learned) |
| `memory.md` | `~/.mneme/projects/<hash>/` | no (per-user log) |
| `_session.json` | `~/.mneme/projects/<hash>/` | no (runtime) |
| `token-ledger.json` | `~/.mneme/projects/<hash>/` | no (per-user stats) |
...
`mneme init` prints a dry-run preview of the 5 changes, prompts `y/N`, then writes:
...
Two artifacts:
1. `~/.claude/mneme-rules.md` — the actual instructions (overwrite-on-upgrade).
2. One-line `@~/.claude/mneme-rules.md` appended to `~/.claude/CLAUDE.md`, wrapped in HTML comment boundary markers.
...
- New shared package `pkg/state` owns all `.mneme/` reads and writes.
...
```
mneme                    → MCP server (default; behavior unchanged)
mneme hook <event>       → event ∈ {pre-read, pre-write, post-write,
                                            session-start, stop}
mneme scan [--force]     → generate anatomy.md
mneme init [flags]       → install hooks + rules
mneme init --uninstall   → reverse install
mneme stats [--waste|--json|--errors]   → ledger / waste / errors report
mneme cerebrum {add|list|remove}        → manage rules
mneme buglog {list|clear}               → manage bug log
mneme hook _<internal>                  → internal subcommands prefixed with _,
...
├── vectordb/           (existing)
├── splitter/           (existing)
├── state/              (NEW — all .mneme/ reads & writes; flock; atomic writes)
...
│   ├── feedback.go     (stderr formatting; "⚡ mneme: ..." prefix)
│   ├── preread.go
│   ├── prewrite.go
│   ├── postwrite.go
│   ├── sessionstart.go
...
│   ├── rules.go        (mneme-rules.md template + @import injection)
│   ├── project.go      (.mneme/ skeleton + .gitignore)
│   └── uninstall.go
...
**Project-local** (`<project>/.mneme/`):

```
.mneme/
├── anatomy.md          <!-- mneme anatomy v1 -->
├── cerebrum.md         <!-- mneme cerebrum v1 -->
├── buglog.json         {"_version": 1, "entries": [...]}
└── .gitignore          (init-written; ignores nothing here, all files commit-friendly)
```

**Global** (`~/.mneme/projects/<hash>/`):

```
~/.mneme/projects/<hash>/
├── memory.md           <!-- mneme memory v1 -->
...
**Existing global** (`~/.mneme/`): unchanged — `config.yaml`, `chromem/`, plus the new `projects/` parent.
...
```
Claude Code spawns: mneme hook pre-read
  ...
  hook/preread.go:
    ...
    3. read anatomy.md (no lock; tolerate stale)
       hit → stderr: "⚡ mneme: src/foo.go — handles auth (~340 tok)"
    4. read _session.json (flock-S, 50ms)
       already present → stderr: "⚡ mneme: src/foo.go already read this session (~340 tok)"
...
hook/sessionstart.go:
    ...
    2. lazy-maintenance check:
       if memory.md > 5000 tokens OR last_consolidation > 7d
         → fork: mneme hook _consolidate &  (background; non-blocking)
    3. exit 0
```
...
**Stderr formatting**: All informational stderr lines carry the `⚡ mneme: ...` prefix so Claude can identify them inside the wrapped error message. Example end result Claude sees: `Failed with non-blocking status code: ⚡ mneme: src/foo.go — handles auth (~340 tok)`.
...
`runtime.lock` lives in `~/.mneme/projects/<hash>/`.
...
1. Plan (dry-run output, no side effects)
   ...
   ├─ [WRITE]  ~/.claude/mneme-rules.md
   ├─ [APPEND] ~/.claude/CLAUDE.md       (one @import line, boundary-marked)
   ├─ [MKDIR]  <project>/.mneme/
   ├─ [WRITE]  <project>/.mneme/.gitignore
   └─ [OPTIONAL] mneme scan
...
3. Execute
   ...
   ├─ mneme-rules.md: atomic write (template includes <!-- mneme-rules v1 -->)
...
### `mneme init --uninstall`

```
1. settings.json: filter out hook entries with _managed_by:"mneme"
2. CLAUDE.md: locate <!-- mneme-managed BEGIN/END --> block; remove
3. Delete ~/.claude/mneme-rules.md
4. Leave <project>/.mneme/ alone (user data); print cleanup hint
```
...
**Manual smoke (in `docs/smoke-checklist.md`, not CI):** fresh Claude Code session → init → trigger reads on anatomy-known files → verify `⚡ mneme: ...` in transcript.
...
**Acceptance:**
- `mneme init` works end-to-end with `y/N` confirmation
...
- `mneme-rules.md` template updated to instruct "consult anatomy first"
...
- Real Claude Code session: `⚡ mneme: ...` visible in transcript
...
- `mneme stats` prints meaningful savings report
...
- `mneme stats --waste`
- `mneme stats --json` for scripting
...
- `mneme scan` on a no-change repo runs in <2s (vs ~30s full)
